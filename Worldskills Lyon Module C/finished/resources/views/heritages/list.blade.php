...
@section('content')
    @php
        $displayTitle = 'Home';
        $isListingMode = ($mode ?? 'listing') === 'listing';

        if ($isListingMode && !empty($listing['relative_path'])) {
            if (!empty($listing['breadcrumbs'])) {
                $lastCrumb = $listing['breadcrumbs'][count($listing['breadcrumbs']) - 1];
                $displayTitle = $lastCrumb['name'];
            } else {
                $displayTitle = trim(str_replace('Folder:', '', $listingTitle));
            }
        } elseif (!$isListingMode) {
